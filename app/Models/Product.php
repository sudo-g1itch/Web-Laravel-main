...
class Product extends Model implements HasMedia
{
    ...
    public function getStockItemsSumQuantityAttribute()
{
    $sum = $this->stocks()->where('status', Status::ACTIVE)->sum('quantity');
    Log::info('Calculating stock items sum quantity', [
        'product_id' => $this->id,
        'sum' => $sum
    ]);
    return $this->stocks()->where('status', Status::ACTIVE)->sum('quantity');
}
