    ...
    public function store(ProductRequest $request): object
    {
        ...
        try {
            DB::transaction(function () use ($request) {
                ...
                $this->product = $product = Product::create($request->validated() + [
                    'slug' => Str::slug($request->name), 
                    'variation_price' => $request->selling_price,
                    'status' => Status::ACTIVE // Ensure the correct status is set
                ]);
                ...
                $stock = Stock::create([
                    ...
                    'status' => Status::ACTIVE, // Ensure this is set correctly
                    'model_type' => Product::class,
                    'model_id' => $product->id,
                    'item_type' => 'default_type', // Provide a value for item_type
                    'item_id' => $product->id, // Use the product ID as the item ID
                ]);
                if ($request->tags) {
